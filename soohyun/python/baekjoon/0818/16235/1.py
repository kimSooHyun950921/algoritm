...
def eat_fertilizer(tree, board):
    ...
    for loc in list(tree.keys()):
        ...
        for idx in range(len(tree[loc])):
            if board[loc[0]][loc[1]] - tree[loc][idx] >= 0:
                board[loc[0]][loc[1]] -= tree[loc][idx]
                heappush(new_tree, tree[loc][idx]+1)
            else:
                dead_tree.append((loc, tree[loc][idx])) 
        tree[loc] = new_tree
    return dead_tree


def change_fertilizer(dead_tree, board):
    for tree in dead_tree:
        loc, year = tree[0], tree[1]
...
def start_finance(board_size, year, fertilizer, tree, board):
    for _ in range(year):
        ...
        #print(board)
        print("=========")
        dead_tree = eat_fertilizer(tree, board)
        print("after")
        print(tree)
        #print(board)
        print(dead_tree)
        #print("===========")
        # 죽은 나무의 양분변화 
        change_fertilizer(dead_tree, board)
